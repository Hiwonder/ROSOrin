...
# @data:2025/03/21
# @author:gcusms
# yolov11目标检测(yolov11 target detection)
...
from example.yolov11_detect.utils import Colors,plot_one_box
...
class Yolov11Node(Node):
    def __init__(self, name):
        ...
        self.yolo_detect = YOLO(( MODE_PATH + "/" + engine), task='detect', verbose=False)
                    ...
                    for result in results:
                        ...
                        if boxes is not None and len(boxes) > 0:
                            for i in range(len(boxes)):
                                box_coords = boxes.xyxy[i].cpu().numpy().astype(int).tolist() 
                                ...
                                if class_id < len(self.classes):
                                    ...
                                    self.get_logger().warn(f"Unknown Class ID: {class_id}")
