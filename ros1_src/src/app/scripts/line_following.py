    ...
    def __call__(self, image, result_image, threshold):
        ...
        for roi in self.rois:
            ...
            if max_contour_area is not None:
                ...
                cv2.drawContours(result_image, [box], -1, (255, 255, 0), 2)  # 画出四个点组成的矩形(draw the rectangle composed of four points)
...
class LineFollowingNode:
    ...
    def image_callback(self, ros_image: Image):
        ...
        if self.debug:
            ...
            self.image_queue.put(cv2.cvtColor(result_image, cv2.COLOR_RGB2BGR))
            print(11111)
        else:
            self.result_publisher.publish(cv2_image2ros(cv2.cvtColor(result_image, cv2.COLOR_RGB2BGR)))  # 发布图像
