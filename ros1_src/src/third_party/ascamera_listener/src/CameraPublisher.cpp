        ...
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                if (it->config_info.is_Registration) {
                    stDepthImage.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                    stDepthCamInfo.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                } else {
                    stDepthImage.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
                    stDepthCamInfo.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
                }
                if (cam_type != AS_SDK_CAM_MODEL_KONDYOR) {
    ...
    if (m_launch_param.color_pcl == false) {
            ...
            for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
                if (it->pCamera == pCamera) {
                    if (cam_type == AS_SDK_CAM_MODEL_KUNLUN_A) {
                        ...
                        if (it->config_info.is_Registration) {
                            pointCloudMsg.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                        } else {
                            pointCloudMsg.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
                        }
        ...
        if (ros::ok()) {
            for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
                if (it->pCamera == pCamera) {
                    if (it->config_info.is_Registration) {
                        pointCloudMsgRgb.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                    } else {
                        pointCloudMsgRgb.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
                    }
                                       ...
                                       AS_CAM_Parameter_s &stParameter, ros::Time time)
{
    ...
    if (ros::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                stRgbCamInfo.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                it->rgb_camera_info_pub.publish(stRgbCamInfo);
                rgbImage.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                it->rgb_img_raw_pub.publish(rgbImage);
                if (m_launch_param.pub_mono8) {
                    mono8Image.header.frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
                                      ...
                                      AS_CAM_Parameter_s &stParameter, ros::Time time)
{
    ...
    if (ros::ok()) {
        for (auto it = imgPubList.begin(); it != imgPubList.end(); it++) {
            if (it->pCamera == pCamera) {
                stIrCamInfo.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
                irImage.header.frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
        ...
        if (it->pCamera == pCamera) {
            ...
            if (it_par != m_cam_type_map.end()) {
                if ((it_par->second == AS_SDK_CAM_MODEL_HP60C) || (it_par->second == AS_SDK_CAM_MODEL_HP60CN)
                    ...
                    rgbTransform.header.frame_id = tf_prefix + "camera_link" + std::to_string(it->index);
                    rgbTransform.child_frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
            ...
            depthTransform.header.frame_id = tf_prefix + "camera_link" + std::to_string(it->index);
            if (it->config_info.is_Registration) {
                depthTransform.child_frame_id = tf_prefix + "color_frame" + std::to_string(it->index);
            } else {
                depthTransform.child_frame_id = tf_prefix + "depth_frame" + std::to_string(it->index);
            }
