...
class JoystickController:
    def __init__(self):
        ...
        self.machine = rospy.get_param('~machine', 'ROSOrin_Mecanum')
    ...
    def axes_callback(self, axes):
        ...
        if self.machine == 'ROSOrin_Mecanum':
        ...
        elif self.machine == 'ROSOrin_Differential':
            twist.linear.y = 0.0
            twist.linear.x = misc.val_map(axes['ly'], -1, 1, -self.max_linear, self.max_linear)
            twist.angular.z = misc.val_map(axes['rx'], -1, 1, -self.max_angular, self.max_angular)
        elif self.machine == 'ROSOrin_Acker':
            twist.linear.x = misc.val_map(axes['ly'], -1, 1, -self.max_linear, self.max_linear)
            steering_angle = misc.val_map(axes['rx'], -1, 1, -math.radians(350/2000*180), math.radians(350/2000*180))
            if twist.linear.x == 0:
                twist.linear.z = 1
