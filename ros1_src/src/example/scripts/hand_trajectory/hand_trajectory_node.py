    ...
    def image_proc(self):
        ...
        while self.running:
            if self.image is not None:
                ...
                bgr_image = image_flip#cv2.cvtColor(image_flip, cv2.COLOR_RGB2BGR)
                bgr_image = cv2.cvtColor(bgr_image, cv2.COLOR_RGB2BGR) if self.camera_type == 'usb_cam' else bgr_image
                self.image = None
                if self.start:
                    try:
