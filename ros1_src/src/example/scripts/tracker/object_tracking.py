...
class KCFNode:
    def __init__(self, name):
        ...
        self.distance = self.DISTANCE  # 选取时停止距离单位cmo
        self.usb_distance_y = 320
    ...
    def image_callback(self, ros_image: Image):
        ...
        rgb_image = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR) if self.camera_type == 'usb_cam' else rgb_image
        try:
        ...
        if self.start_circle:
            ...
            if ok and min(bbox) > 0:
                ...
                if self.camera_type != 'usb_cam':
                    ...
                    self.pid_d.SetPoint = self.usb_distance_y
                    if abs(center[1] - self.usb_distance_y) < 10:
                        center[1] = self.usb_distance_y
                    self.pid_d.update(center[1])  #更新pid
                    tmp = self.go_speed - self.pid_d.output
                    self.linear_x = tmp
                    if tmp > 0.3:
                        self.linear_x = 0.3
